...
with params as (
select 90::int as lookback_days
),
as_of as (
select coalesce(max(order_date), current_date) as as_of_date
from sales
),
date_spine AS (
...
avg(qty) filter (
where dt > (select as_of_date from as_of) - INTERVAL '30 days'
) as avg_daily_units_30d
avg(qty) filter (
where dt > (select as_of_date from as_of) - INTERVAL '7 days'
